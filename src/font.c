...
struct Font *font_allocate(const char *fp) {
    ...
    font->small = TTF_OpenFont(fp, 12);
    font->medium = TTF_OpenFont(fp, 18);
    font->large = TTF_OpenFont(fp, 26);
...
void font_draw(struct Font *font, const char *text, int x, int y, int size, int *width, int *height) {
    TTF_Font *f;
    ...
    SDL_Surface *surf = TTF_RenderText_Blended(f, text, (SDL_Color){255,255,255,255});
